...
def predict_from_uart(ser,model): # this function is used to detect activity
    ...
    try:
                    ...
                    if len(values) == 7:
                        ...
                        if check_recog and z < queue_size - windowSlide:
                            ...
                            if i >= windowSlide and test >= test_counter:
                                ...
                                if np.max(rounded_result) > 0.9:
                                    predicted_values.append(np.max(rounded_result) - 0.3)
                                else:
                                    if np.max(rounded_result) > 0.8:
                                        predicted_values.append(np.max(rounded_result) - 0.2)
                                    else:
                                        if np.max(rounded_result) > 0.69:
                                            predicted_values.append(np.max(rounded_result) - 0.1)
                                        else: predicted_values.append(np.max(rounded_result))
        ...
        plt.tight_layout()
        plt.savefig('./graph/all_data_model.png')
        plt.show()
...
def run_experiment():
    ...
    while True:
        ...
        if mode == 1:
            ...
            model.load_weights('./model/all_data_model.h5')
            break
        if mode == 2:
